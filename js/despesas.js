...
var Despesas = {
  ...
  salvar: async function () {
    ...
    var idEmEdicao = (
      Despesas.editando &&
      Despesas.editando.id
    ) ? Despesas.editando.id : null;
    ...
    try {
      ...
      if (
        ...
        /* Compatibilidade de segurança sem offline.js. */
        var respostaDireta;
      ...
      if (resultado.pendente) {
        ...
        if (idEmEdicao) {
          Despesas.lista = (Despesas.lista || []).map(function (item) {
            return item.id === idEmEdicao
              ? Object.assign({}, item, localReg)
              : item;
          });
        } else {
          Despesas.lista = [localReg].concat(Despesas.lista || []);
        }

        App.toast(
          idEmEdicao
            ? 'Alteração salva no aparelho e aguardando sincronização.'
            : 'Despesa salva no aparelho e aguardando sincronização.',
          'ok'
        );
      } else {
        App.toast(
          idEmEdicao ? 'Atualizado!' : 'Despesa registrada!',
          'ok'
        );
      }
      ...
      App.toast(
        navigator.onLine
          ...
          : 'Não foi possível guardar a despesa neste aparelho.',
        'erro'
      );
    } finally {
