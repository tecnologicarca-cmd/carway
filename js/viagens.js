/* APP_VERSION: v10.5 - multi-energetico */
/* =====================================================================
   CARWAY v16 - VIAGENS
...
chamarRoutes: function (origem, destino, idaVolta, emissionType) {
  ...
  _energeticosDoVeiculo: function (veic) {
    if (!veic) return ['Gasolina'];
    var txt = String(veic.energeticos || '').trim();
    if (txt) {
  ...
  _iconeCombustivelInfo: function (combustivel) {
    return Viagens._infoEnergetico(combustivel);
  },
  ...
  _veiculoBlocoHTML: function (veic, comId) {
    ...
    var energetico = Viagens.plano.energeticoSelecionado || Viagens._energeticosDoVeiculo(veic)[0];
    var info = Viagens._infoEnergetico(energetico);
  ...
  _cardsEnergeticosHTML: function (veic, atual) {
    var lista = Viagens._energeticosDoVeiculo(veic);
    if (lista.indexOf(atual) === -1) atual = lista[0];
    return lista.map(function (e) {
      var i = Viagens._infoEnergetico(e), sel = e === atual;
      return '<button type="button" onclick="Viagens.selecionarEnergeticoPlano(\'' + e + '\')" style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;min-height:82px;padding:10px 5px;border-radius:12px;font-family:inherit;cursor:pointer;background:' + (sel ? i.cor + '26' : 'var(--bg2,#111c33)') + ';border:2px solid ' + (sel ? i.cor : 'var(--linha,#26365c)') + ';color:' + (sel ? i.cor : 'var(--txt2,#93a4c8)') + '"><span class="ms" style="font-size:25px;color:' + i.cor + '">' + i.icone + '</span><b style="font-size:11.5px">' + e + '</b><small>' + i.unidade + '</small></button>';
    }).join('');
  },

  selecionarEnergeticoPlano: function (energetico) {
    var sel = document.getElementById('plVeiculo');
    var veic = Viagens.veiculos.filter(function (x) { return sel && x.id === sel.value; })[0];
    if (!veic || Viagens._energeticosDoVeiculo(veic).indexOf(energetico) === -1) return;
    Viagens.plano.energeticoSelecionado = energetico;
    var cards = document.getElementById('plEnergeticos');
    if (cards) cards.innerHTML = Viagens._cardsEnergeticosHTML(veic, energetico);
    var card = document.getElementById('cardVeicPlano');
    if (card) card.outerHTML = Viagens._veiculoBlocoHTML(veic, 'cardVeicPlano');
    Viagens._aplicarEnergeticoNoFormulario(veic, energetico, true);
  },

  _aplicarEnergeticoNoFormulario: function (veic, energetico, atualizarPreco) {
    var info = Viagens._infoEnergetico(energetico);
    var capacidade = Viagens._capacidadeEnergetico(veic, energetico);
    var lCons = document.getElementById('plLabelConsumo');
    var lCap = document.getElementById('plLabelCapacidade');
    var lNivel = document.getElementById('plLabelNivel');
    var lPreco = document.getElementById('plLabelPreco');
    ...
    if (atualizarPreco) {
      ...
      var medio = Viagens.precoMedioCombustivel(veic.id, energetico);
      if (preco) preco.value = medio > 0 ? medio : '';
    }
  ...
  calcularTotais: function (viagemId) {
    var combustivel = 0, desp = 0, qtdDesp = 0, manut = 0, qtdManut = 0;
    var quantidades = {};
    Viagens.abastecimentos.forEach(function (a) {
      ...
      var u = a.unidade || Viagens._infoEnergetico(a.combustivel).unidade;
      combustivel += val;
      quantidades[u] = (quantidades[u] || 0) + (Number(a.litros) || 0);
    });
  ...
  precoMedioCombustivel: function (veiculoId, energetico) {
    var lista = (Viagens.abastecimentos || []).filter(function (a) {
      return a.veiculoId === veiculoId && (!energetico || a.combustivel === energetico) && Number(a.precoLitro) > 0;
    });
...
abrirPlanejador: function (idExistente) {
  ...
  renderPlanejador: function (viagemExistente) {
    ...
    if (!energeticoSalvo && v.rota) { try { energeticoSalvo = JSON.parse(v.rota).energetico || null; } catch (e) {} }
    var energeticoSel = energeticoSalvo && listaEnergeticos.indexOf(energeticoSalvo) > -1 ? energeticoSalvo : (Viagens.plano.energeticoSelecionado && listaEnergeticos.indexOf(Viagens.plano.energeticoSelecionado) > -1 ? Viagens.plano.energeticoSelecionado : listaEnergeticos[0]);
    Viagens.plano.energeticoSelecionado = energeticoSel;
    var infoEnergia = Viagens._infoEnergetico(energeticoSel);
    var precoMedio = Viagens.precoMedioCombustivel(vSel, energeticoSel);
    var precoInicial = v.precoLitro || precoMedio || 0;
    ...
    var html =
      ...
      '</div>' +
      '<div class="linha-2">' +
        '<div class="campo-form"><label id="plLabelConsumo">Consumo (' + infoEnergia.consumo + ')</label>' +
          '<input type="number" id="plKmL" step="0.1" placeholder="11.5" value="' + (v.kmL || 11.5) + '" oninput="Viagens.previewAutonomia()">' +
        '</div>' +
        '<div class="campo-form"><label id="plLabelCapacidade">' + infoEnergia.capacidade + ' (' + infoEnergia.unidade + ')</label>' +
          '<input type="number" id="plTanque" step="0.5" placeholder="50" value="' + (v.tanque || Viagens._capacidadeEnergetico(veic, energeticoSel) || 0) + '" oninput="Viagens.previewAutonomia()">' +
        '</div>' +
      '</div>' +
      '<div class="linha-2">' +
        '<div class="campo-form"><label id="plLabelNivel">' + infoEnergia.capacidade + ' agora</label>' +
  ...
  trocarVeiculoPlano: function () {
    ...
    if (!veic) return;
    var lista = Viagens._energeticosDoVeiculo(veic);
    Viagens.plano.energeticoSelecionado = lista[0] || 'Gasolina';
    var cards = document.getElementById('plEnergeticos');
    if (cards) cards.innerHTML = Viagens._cardsEnergeticosHTML(veic, Viagens.plano.energeticoSelecionado);
    var card = document.getElementById('cardVeicPlano');
    if (card) card.outerHTML = Viagens._veiculoBlocoHTML(veic, 'cardVeicPlano');
    Viagens._aplicarEnergeticoNoFormulario(veic, Viagens.plano.energeticoSelecionado, true);
  },
...
nomeApp: function (app) {
  ...
  if (p) {
    ...
    if (p.energetico) Viagens.plano.energeticoSelecionado = p.energetico;
  }
  ...
  buscarRotas: function () {
    ...
    var energetico = Viagens.plano.energeticoSelecionado || Viagens._energeticosDoVeiculo(veic)[0];
    var infoEnergia = Viagens._infoEnergetico(energetico);
    Viagens.plano.veiculoSelecionado = veic;
    Viagens.plano.parametrosAutonomia = { kmL: kmL, tanque: tanque, nivel: nivel, reserva: reserva, preco: preco, energetico: energetico, unidade: infoEnergia.unidade, categoria: infoEnergia.categoria };
  ...
  _encontrarPostoParaPosicao: function (kmIdeal, limiteInferior, coords, kmSo, idaVolta) {
    ...
    function buscar(km, raio) {
      ...
      return (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.categoria === 'eletrico' ? Viagens._buscarPontosRecargaOCM(pos[0], pos[1]) : Viagens.chamarPlaces(pos[0], pos[1], raio, 5, 'gas_station')).then(function (locais) {
  ...
  buscarPostosParaParadasDaRota: function (paradas, rota) {
    ...
    function tentarPontoIdeal(p) {
      return (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.categoria === 'eletrico' ? Viagens._buscarPontosRecargaOCM(p.lat, p.lon) : Viagens.chamarPlaces(p.lat, p.lon, 8000, 5, 'gas_station')).then(function (locais) {
    ...
    return processarEmLotes(paradas, 5, tentarPontoIdeal).then(function () {
      ...
      function tentarRaioMaior(p) {
        return (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.categoria === 'eletrico' ? Viagens._buscarPontosRecargaOCM(p.lat, p.lon) : Viagens.chamarPlaces(p.lat, p.lon, 25000, 5, 'gas_station')).then(function (locais) {
          if (locais.length) { Viagens._aplicarPostoEncontrado(p, locais); }
          return null;
        }).catch(function () { return null; });
      }
  ...
  _buscarPostoRetroativo: function (p, coords, kmSo, idaVolta, limiteInferior) {
    ...
    function tentar() {
      ...
      return (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.categoria === 'eletrico' ? Viagens._buscarPontosRecargaOCM(pos[0], pos[1]) : Viagens.chamarPlaces(pos[0], pos[1], 12000, 5, 'gas_station')).then(function (locais) {
        if (locais.length) {
...
html += '<div class="form-acoes-viagem" style="margin-top:20px"><button class="btn-cancelar-form" onclick="Viagens.voltarParaFormularioMantendoRotas()"><span class="ms">arrow_back</span> Voltar</button></div>';
  ...
  criarViagemDaRota: function (idx) {
    ...
    var reg = {
      ...
      rota: JSON.stringify({
        ...
        energetico: Viagens.plano.energeticoSelecionado || 'Gasolina',
        unidadeQuantidade: (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.unidade) || 'L',
        consumo: (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.kmL) || 0,
        capacidade: (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.tanque) || 0
      }),
      obs: obs
    };
    var paradas = dados ? dados.paradas : [];
    ...
    var paradasReg = paradas.map(function (p) {
      return {
        ...
        energetico: (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.energetico) || 'Gasolina',
        unidadeQuantidade: (Viagens.plano.parametrosAutonomia && Viagens.plano.parametrosAutonomia.unidade) || 'L',
  ...
  abrirBuscaPostos: function () {
    ...
    if (veicSelecionado) {
      Viagens._buscarPostosPara(Viagens._categoriaCombustivel(Viagens.plano.energeticoSelecionado || veicSelecionado.combustivel));
      return;
    }
...
paradaSalvaHTML: function (p) {
  ...
  if (concluida && vinculada) {
    ...
    selo =
      '<span class="parada-selo" style="color:#22c55e;border-color:#22c55e;background:rgba(34,197,94,.10)">' +
    ...
    selo =
      '<span class="parada-selo" style="color:#ef4444;border-color:#ef4444;background:rgba(239,68,68,.10)">' +
        '<span class="ms" style="color:#ef4444">report</span>' +
        '<b>SEM POSTO</b>' +
      '</span>';
    ...
    selo =
      '<span class="parada-selo" style="color:#f59e0b;border-color:#f59e0b;background:rgba(245,158,11,.10)">' +
        '<span class="ms" style="color:#f59e0b">schedule</span>' +
        '<b>ANTECIPADA</b>' +
      '</span>';
  ...
  if (concluida && vinculada) cor = '#60a5fa';
  else if (concluida) cor = '#22c55e';
  else if (ignorada) cor = '#94a3b8';
  else if (semPosto) cor = '#ef4444';
  else if (antecipada) cor = '#f59e0b';

  var titulo = p.postoNome || ('Parada ' + p.ordem);

  var estrelas = (p.postoRating > 0)
    ? '<span style="color:#f59e0b;font-size:11px;font-weight:700;margin-left:6px">' +
        '<span class="ms" style="font-size:13px;vertical-align:middle">star</span>' +
        Number(p.postoRating).toFixed(1) +
      '</span>'
    : '';
  ...
  if (semPosto) {
    ...
    aviso =
      ...
      'Nenhum posto mapeado nesta faixa segura.' +
      '</small>';
    ...
    aviso =
      ...
      'Antecipada em ' +
      Math.round((p.kmOriginalPrevisto - p.kmPrevisto) * 10) / 10 +
      ' km' +
      '</small>';
...
if (concluida) {
  ...
  if (!concluida && !ignorada) {
    ...
    acoes =
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:10px">' +
        '<button class="btn-novo-sec" onclick="Viagens.formConcluirParada(\'' + p.id + '\')">' +
    ...
    acoes =
      '<div style="margin-top:10px">' +
        '<button class="btn-novo-sec" style="width:100%" onclick="Viagens.reabrirParada(\'' + p.id + '\')">' +
          '<span class="ms" style="color:#f59e0b">undo</span> ' +
          (concluida ? 'Desfazer' : 'Reativar') +
        '</button>' +
      '</div>';
  }

  return (
    '<div class="parada-item ' + classeStatus + '" style="--cor-parada:' + cor + '">' +
      '<div style="display:flex;align-items:flex-start;gap:10px">' +
        '<div class="parada-num" style="background:' + cor + '22;color:' + cor + ';flex:none">' +
          p.ordem +
        '</div>' +
        '<div class="parada-info" style="flex:1;min-width:0">' +
          selo +
          '<b style="display:block">' + App.esc(titulo) + '</b>' +
          estrelas +
          aviso +
          linhaValores +
          acoes +
    ...
    var energeticoParada = p.energetico || Viagens._energeticosDoVeiculo(veiculo)[0] || 'Gasolina';
    var infoParada = Viagens._infoEnergetico(energeticoParada);
    var unidade = p.unidadeQuantidade || infoParada.unidade;
    var html =
      '<div class="aviso info" style="margin-bottom:16px">' +
    ...
    App.abrirModal('Registrar abastecimento', html, function () {
      ...
      sb.from('abastecimentos').insert(reg).then(function (r1) {
        ...
        if (App._painelRaw) App._painelRaw = null;
        App.toast('Parada ' + p.ordem + ' concluÃ­da', 'ok');
  ...
  vincularAbastParada: function (paradaId) {
    ...
    sb.from('abastecimentos').select('*').eq('viagemId', d.viagem.id).is('paradaId', null).then(function (r) {
      var parada = d.paradas.filter(function (x) { return x.id === paradaId; })[0] || {};
      var lista = (r.data || []).filter(function (a) { return !parada.energetico || a.combustivel === parada.energetico; });
      if (!lista.length) {
